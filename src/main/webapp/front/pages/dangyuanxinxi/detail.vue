		...
		<view class="container">
				...
				<swiper-item v-for="(swiper,index) in swiperList" :key="index">
					...
					<view class="text" :style='{"color":"rgba(0, 0, 0, 1)","textAlign":"left","fontSize":"28rpx"}'>{{detail.nianling}}</view>
                				                				...
                				                				<view class="detail-list-item" :style='{"boxShadow":"0 0 0px rgba(0,0,0,.3)","margin":"0 0 2rpx 0","borderColor":"rgba(171, 174, 176, 1)","borderRadius":"0","borderWidth":"2rpx","borderStyle":"none none solid none","height":"68rpx"}'>
					<view class="lable" :style='{"width":"auto","fontSize":"28rpx","color":"rgba(6, 6, 6, 1)","textAlign":"left"}'>提交日期：</view>
					<view class="text" :style='{"color":"rgba(0, 0, 0, 1)","textAlign":"left","fontSize":"28rpx"}'>{{detail.tijiaoriqi}}</view>
				</view>
			...
			<view class="bottom-content cu-bar bg-white tabbar border shop">
				...
				<view style="text-align: right;">
																				<button v-if="isAuth('dangyuanxinxi','添加政审')" @tap="onAcrossTap('zhengshenjieguo')" class="cu-btn bg-brown round shadow-blur" style="margin-right: 10upx;min-width: 180upx;" :style="{width:'auto',borderRadius:'8rpx',height:'80rpx',fontSize:'28rpx',color:'#fff',backgroundColor:btnColor[0+6],borderColor:btnColor[0+6]}">添加政审</button>
															<button v-if="isAuth('dangyuanxinxi','添加谈话记录')" @tap="onAcrossTap('dangyuantanhuajilu')" class="cu-btn bg-brown round shadow-blur" style="margin-right: 10upx;min-width: 180upx;" :style="{width:'auto',borderRadius:'8rpx',height:'80rpx',fontSize:'28rpx',color:'#fff',backgroundColor:btnColor[1+6],borderColor:btnColor[1+6]}">添加谈话记录</button>
															<button v-if="isAuth('dangyuanxinxi','添加党课培训结果')" @tap="onAcrossTap('dangkepeixunjieguo')" class="cu-btn bg-brown round shadow-blur" style="margin-right: 10upx;min-width: 180upx;" :style="{width:'auto',borderRadius:'8rpx',height:'80rpx',fontSize:'28rpx',color:'#fff',backgroundColor:btnColor[2+6],borderColor:btnColor[2+6]}">添加党课培训结果</button>
															<button v-if="isAuth('dangyuanxinxi','转正')" @tap="onAcrossTap('zhuanzhengguanli')" class="cu-btn bg-brown round shadow-blur" style="margin-right: 10upx;min-width: 180upx;" :style="{width:'auto',borderRadius:'8rpx',height:'80rpx',fontSize:'28rpx',color:'#fff',backgroundColor:btnColor[3+6],borderColor:btnColor[3+6]}">转正</button>
																														                    										                </view>
			</view>
		</view>
...
<script>
	export default {
		...
		methods: {
			// 支付
			onPayTap(){
				uni.setStorageSync('paytable','dangyuanxinxi');
			...
			onAcrossTap(tableName){
     			uni.setStorageSync('crossTable','dangyuanxinxi');
				uni.setStorageSync(`crossObj`, this.detail);
				this.$utils.jump(`../${tableName}/add-or-update?cross=true`);
			},
			// 获取详情
			async init(){
				let res = await this.$api.info('dangyuanxinxi', this.id);
			...
			// 添加评论
			onCommentTap() {
				this.$utils.jump(`../discussdangyuanxinxi/add-or-update?refid=${this.id}`)
			},
			// 点赞
			async zan() {
				let _this = this;
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							...
							await _this.$api.update('dangyuanxinxi', _this.detail);
			...
			async onFinishTap() {
				...
				await this.$api.update('dangyuanxinxi', this.detail);
				this.$utils.msg('审核成功');
