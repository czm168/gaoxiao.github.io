<template>
	<view class="content">
		<form class="app-update-pv">
				...
				<view :style='{"width":"170rpx","fontSize":"28rpx","color":"rgba(0, 0, 0, 1)","textAlign":"left"}' class="title">身份类型</view>
				<picker @change="shenfenleixingChange" :value="shenfenleixingIndex" :range="shenfenleixingOptions">
					<view :style='{"boxShadow":"0 0 4rpx rgba(0,0,0,.6) inset","backgroundColor":"rgba(255, 255, 255, 0)","borderColor":"rgba(0,0,0,.6)","borderRadius":"16rpx","color":"rgba(23, 152, 242, 1)","textAlign":"left","borderWidth":"2rpx","fontSize":"28rpx","borderStyle":"solid","height":"88rpx"}' class="uni-input">{{ruleForm.shenfenleixing?ruleForm.shenfenleixing:"请选择身份类型"}}</view>
				</picker>
			</view>
												<view :style='{"boxShadow":"0 0 16rpx rgba(0,0,0,.3)","backgroundColor":"rgba(255, 255, 255, 0.25)","borderColor":"#ccc","margin":"0 0 20rpx 0","borderWidth":"0","borderStyle":"solid","height":"108rpx"}' class="cu-form-group select">
				...
				<input :style='{"boxShadow":"0 0 4rpx rgba(0,0,0,.6) inset","backgroundColor":"rgba(255, 255, 255, 0)","borderColor":"rgba(0,0,0,.6)","borderRadius":"16rpx","color":"rgba(23, 152, 242, 1)","textAlign":"left","borderWidth":"2rpx","fontSize":"28rpx","borderStyle":"solid","height":"110rpx"}' :disabled="ro.nianling" v-model="ruleForm.nianling" placeholder="年龄"></input>
												...
												<view :style='{"boxShadow":"0 0 16rpx rgba(0,0,0,.3)","backgroundColor":"rgba(255, 255, 255, 0.25)","borderColor":"#ccc","margin":"0 0 20rpx 0","borderWidth":"0","borderStyle":"solid","height":"108rpx"}' class="cu-form-group select">
				<view :style='{"width":"170rpx","fontSize":"28rpx","color":"rgba(0, 0, 0, 1)","textAlign":"left"}' class="title">提交日期</view>
				<picker mode="date" :value="ruleForm.tijiaoriqi" @change="tijiaoriqiChange">
					<view :style='{"boxShadow":"0 0 4rpx rgba(0,0,0,.6) inset","backgroundColor":"rgba(255, 255, 255, 0)","borderColor":"rgba(0,0,0,.6)","borderRadius":"16rpx","color":"rgba(23, 152, 242, 1)","textAlign":"left","borderWidth":"2rpx","fontSize":"28rpx","borderStyle":"solid","height":"88rpx"}' class="uni-input">{{ruleForm.tijiaoriqi?ruleForm.tijiaoriqi:"请选择提交日期"}}</view>
				</picker>
			</view>
	...
	export default {
				...
				ruleForm: {
												zhanghao: '',
																...
																userid: '',
																												...
																												shenfenleixingOptions: [],
				shenfenleixingIndex: 0,
																xingbieOptions: [],
				xingbieIndex: 0,
																																																				// 登陆用户信息
				user: {},
		...
		async onLoad(options) {
			...
			this.user = res.data;
						...
						// ss读取
																								...
																								// 自定义下拉框值
			this.shenfenleixingOptions = "积极分子,发展对象,预备党员,正式党员".split(',')
												// 自定义下拉框值
			...
			if (options.id) {
				...
				// 获取信息
				res = await this.$api.info(`dangyuanxinxi`, this.ruleForm.id);
			...
			if(options.cross){
				...
				for (var o in obj){
					...
					if(o=='shenfenleixing'){
					this.ruleForm.shenfenleixing = obj[o];
					this.ro.shenfenleixing = true;
					continue;
					}
					if(o=='xingbie'){
		...
		methods: {
			...
			// 多级联动参数
																																													...
																																													tijiaoriqiChange(e) {
				this.ruleForm.tijiaoriqi = e.target.value;
				this.$forceUpdate();
			},
																		
																																																									
																					// 下拉变化
			shenfenleixingChange(e) {
				this.shenfenleixingIndex = e.target.value
				this.ruleForm.shenfenleixing = this.shenfenleixingOptions[this.shenfenleixingIndex]
			},
												// 下拉变化
			xingbieChange(e) {
			...
			async onSubmitTap() {
					...
					return
																																				...
																																				if(this.ruleForm.id){
					await this.$api.update(`dangyuanxinxi`, this.ruleForm);
				}else{
					await this.$api.add(`dangyuanxinxi`, this.ruleForm);
				}
