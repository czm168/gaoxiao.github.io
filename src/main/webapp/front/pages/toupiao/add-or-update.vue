<template>
	<view class="content">
		<form class="app-update-pv">
				...
				<picker @change="piaoshuChange" :value="piaoshuIndex" :range="piaoshuOptions">
					<view :style='{"boxShadow":"0 0 4rpx rgba(0,0,0,.6) inset","backgroundColor":"rgba(255, 255, 255, 0)","borderColor":"rgba(0,0,0,.6)","borderRadius":"16rpx","color":"rgba(23, 152, 242, 1)","textAlign":"left","borderWidth":"2rpx","fontSize":"28rpx","borderStyle":"solid","height":"88rpx"}' class="uni-input">{{ruleForm.piaoshu?ruleForm.piaoshu:"请选择票数"}}</view>
				</picker>
			</view>
	...
	export default {
		...
		async onLoad(options) {
			...
			if (options.id) {
				...
				// 获取信息
				res = await this.$api.info(`toupiao`, this.ruleForm.id);
		...
		methods: {
			...
			// 多级联动参数
																											...
																											// 下拉变化
			piaoshuChange(e) {
				this.piaoshuIndex = e.target.value
				this.ruleForm.piaoshu = this.piaoshuOptions[this.piaoshuIndex]
			},
			...
			async onSubmitTap() {
				...
				await this.$api.update(`${table}`, obj);
					...
					return
																																				...
																																				if(this.ruleForm.id){
					await this.$api.update(`toupiao`, this.ruleForm);
				}else{
					await this.$api.add(`toupiao`, this.ruleForm);
				}
