	...
	<mescroll-uni @init="mescrollInit" :up="upOption" :down="downOption" @down="downCallback" @up="upCallback" :bottom="100">
		<view class="container">
					...
					姓名：{{detail.xingming}}
                                								                								                				...
                                								                								                				<view class="detail-list-item" :style='{"boxShadow":"0 0 0px rgba(0,0,0,.3)","margin":"0 0 2rpx 0","borderColor":"rgba(171, 174, 176, 1)","borderRadius":"0","borderWidth":"2rpx","borderStyle":"none none solid none","height":"68rpx"}' @tap="download(detail.shenqingwenjian)" >
					<view class="lable" :style='{"width":"auto","fontSize":"28rpx","color":"rgba(6, 6, 6, 1)","textAlign":"left"}'>申请文件：</view>
				...
				<view class="content" :style='{"color":"#333","fontSize":"28rpx"}'>
					<rich-text style="font-size: 33upx;line-height: 50upx;letter-spacing: 5upx;" :nodes="detail.shenqingliyou"></rich-text>
				</view>
			</view>
			...
			<view class="bottom-content cu-bar bg-white tabbar border shop">
				...
				<view style="text-align: right;">
																				<button v-if="isAuth('pingyoushenqing','发布投票')" @tap="onAcrossTap('minzhutoupiao')" class="cu-btn bg-brown round shadow-blur" style="margin-right: 10upx;min-width: 180upx;" :style="{width:'auto',borderRadius:'8rpx',height:'80rpx',fontSize:'28rpx',color:'#fff',backgroundColor:btnColor[0+6],borderColor:btnColor[0+6]}">发布投票</button>
																														                    										<button v-if="isAuth('pingyoushenqing','审核')" @tap="onSHTap" style="margin-right: 10upx;" class="cu-btn bg-red round shadow-blur" :style="{width:'auto',borderRadius:'8rpx',height:'80rpx',fontSize:'28rpx',color:'#fff',backgroundColor:btnColor[4],borderColor:btnColor[4]}">审核</button>
										                </view>
			</view>
		</view>
...
<script>
	export default {
		...
		methods: {
			// 支付
			onPayTap(){
				uni.setStorageSync('paytable','pingyoushenqing');
			...
			onAcrossTap(tableName){
     			uni.setStorageSync('crossTable','pingyoushenqing');
				uni.setStorageSync(`crossObj`, this.detail);
				this.$utils.jump(`../${tableName}/add-or-update?cross=true`);
			},
			// 获取详情
			async init(){
				let res = await this.$api.info('pingyoushenqing', this.id);
			...
			// 添加评论
			onCommentTap() {
				this.$utils.jump(`../discusspingyoushenqing/add-or-update?refid=${this.id}`)
			},
			// 点赞
			async zan() {
				let _this = this;
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							...
							await _this.$api.update('pingyoushenqing', _this.detail);
			...
			async onFinishTap() {
				...
				await this.$api.update('pingyoushenqing', this.detail);
				this.$utils.msg('审核成功');
