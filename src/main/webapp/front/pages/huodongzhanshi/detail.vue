	...
	<mescroll-uni @init="mescrollInit" :up="upOption" :down="downOption" @down="downCallback" @up="upCallback" :bottom="100">
		<view class="container">
				...
				<view class="content" :style='{"color":"#333","fontSize":"28rpx"}'>
					<rich-text style="font-size: 33upx;line-height: 50upx;letter-spacing: 5upx;" :nodes="detail.huodongneirong"></rich-text>
				</view>
			</view>
...
<script>
	export default {
		...
		methods: {
			// 支付
			onPayTap(){
				uni.setStorageSync('paytable','huodongzhanshi');
			...
			onAcrossTap(tableName){
     			uni.setStorageSync('crossTable','huodongzhanshi');
				uni.setStorageSync(`crossObj`, this.detail);
				this.$utils.jump(`../${tableName}/add-or-update?cross=true`);
			},
			// 获取详情
			async init(){
				let res = await this.$api.info('huodongzhanshi', this.id);
			...
			// 添加评论
			onCommentTap() {
				this.$utils.jump(`../discusshuodongzhanshi/add-or-update?refid=${this.id}`)
			},
			// 点赞
			async zan() {
				let _this = this;
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							...
							await _this.$api.update('huodongzhanshi', _this.detail);
			...
			async onFinishTap() {
				...
				await this.$api.update('huodongzhanshi', this.detail);
				this.$utils.msg('审核成功');
